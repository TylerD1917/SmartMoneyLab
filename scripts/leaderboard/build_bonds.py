"""
Leaderboard — Step 1a: serie TOTAL RETURN dei bond USA a maturita' costante (CMT).
Ricostruisce il rendimento total-return mensile di un titolo "benchmark" a
maturita' fissa a partire dal solo rendimento a scadenza, rivalutando ogni mese
un par bond che invecchia di 1/12 di anno e viene riprezzato al nuovo rendimento.
Cattura carry + effetto prezzo (duration+convessita').

Bucket:
  US  20y / 10y / 2y   da FRED DGS20, DGS10 (cache), DGS2   (USD, dal 2016)

La gamba EUR NON e' piu' sintetica: arriva da ETF governativi euro (IBGS/IBGZ/IBGL)
scaricati in fetch_data.py e assemblati direttamente in build_blocks.py — cosi' si
aggiorna da sola via yfinance, senza l'export manuale di REPORT_BancaItalia.xlsx.

Output: data/processed/leaderboard_bonds.csv  (indici TR base 100, USD, mensili)
"""
import os
...
try:
    ...
    print("\n[i] corr_tlt.csv assente, salto il check TLT")
print("2022 (anno del crollo obbligazionario) — rendimento annuo US:")
for k in ["us_20y","us_2y"]:
